...
contract CundinaBlockSecure is Ownable2Step, ReentrancyGuard {
    ...
    function joinBlock(address member) external onlyRegistry nonReentrant {
        ...
        // NO token transfer - registration fee already covers the contribution
        // The fee was collected during registerUser() and sent to treasurySafe
...
contract BlockRegistryFactory is Ownable2Step, ReentrancyGuard {
    ...
    // Registration fee is now dynamic per level (uses levelCfg[level].contributionAmount)
    ...
    // Removed setRegistrationFee - registration fee is now dynamic per level
    ...
    /// @return needsPayment True if user hasn't registered yet and needs to pay
    /// @return currentLevel The user's current level (0 if not registered)
    /// @return hasBlockAtLevel True if user already has a block at their current level
    ...
    /// @notice Register and create block in one transaction. Only charges if not already registered.
    /// @dev If user is already registered (paid before), this will NOT charge again.
    /// @param user The user address
    /// @param referrer The referrer address (can be address(0))
    /// @param level The level to register for (1-7)
    /// @return blockAddress The address of the created block
        ...
        returns (address blockAddress) 
    {
        ...
        // Check if user already has a block at this level
        require(myBlockAtLevel[user][level] == address(0), "already has block");

        // Only charge if user is NOT already registered
        if (userLevel[user] == 0) {
            // First time registration - charge the fee
            uint256 fee = levelCfg[level].contributionAmount;
            ...
            // User already registered - NO charge, just verify level matches
    ...
    /// @notice Calculate GROSS for a block (total contributions before any fees)
    /// @dev This is requiredMembers × contributionAmount
    ///      For Level 1: 9 × 20 = 180 USDT gross
    ...
    /// @notice Calculate NET for a block (what's actually in Treasury after 10% PI fee on each contribution)
    /// @dev Each member contributes X, but 10% goes to PI at join time, so 90% goes to Treasury
    ///      For Level 1: 9 × (20 × 0.9) = 9 × 18 = 162 USDT net in Treasury
    function netForBlock(address blockAddr) public view returns (uint256) {
        uint256 gross = grossForBlock(blockAddr);
        // 10% was taken as PI fee during each join, so Treasury only has 90%
        return (gross * 90) / 100;
    }
...
contract SafeTreasuryPayoutModule is ReentrancyGuard {
    ...
    address public immutable piWallet; // Propiedad Intelectual wallet for 10% dispersion
    ...
    /// @notice Disperse the registration fee: 10% to PI wallet, 90% stays in Treasury
    /// @param user The user who paid the registration fee
    /// @param level The level for which the fee was paid
    /// @dev Call this after a successful registration to disperse the 10% to PI
    function disperseRegistrationFee(address user, uint256 level) external nonReentrant {
        require(user != address(0), "user=0");
        require(level >= 1 && level <= 7, "invalid level");

        // Get the registration fee for this level
        ...
        // Transfer 10% from Treasury to PI wallet
        _safeTransfer(piWallet, piAmount);
        
        // The remaining 90% stays in Treasury (no transfer needed)
        uint256 treasuryAmount = totalFee - piAmount;

        emit RegistrationFeeDispersed(user, level, piAmount, treasuryAmount);
    }

    // Removed onlyTreasurySafe modifier - anyone can call cashout/advance
    // The module will execute transfers FROM the Safe via execTransactionFromModule

    /// @notice Cashout - user withdraws all accumulated funds and loses progression opportunity
    /// @dev The 10% platform fee was ALREADY deducted when each member joined (at registration)
    ///      So the Treasury only holds the NET amount: requiredMembers × (contribution × 0.9)
    ///      For Level 1: 9 × 18 = 162 USDT net in Treasury
    ///      User receives all 162 USDT but cannot advance to next level
    function cashout(address blockAddr, address center, address payoutTo) external nonReentrant {
        require(center != address(0), "center=0");
        require(center != treasurySafe, "center=treasury");
        require(payoutTo != address(0), "payout=0");

        // Net accumulated in Treasury (fee was already taken at join time)
        uint256 accumulated = registry.netForBlock(blockAddr);
        
        // Full payout to user (no additional fee)
    ...
    /// @notice Advance to the next level
    /// @dev CORRECTED Distribution logic:
    ///      The 10% platform fee was ALREADY deducted when each member joined.
    ///      Treasury holds NET: requiredMembers × (contribution × 0.9)
    ///      For Level 1: 9 × 18 = 162 USDT net in Treasury
    ///      
    ///      When advancing from L1 → L2:
    ///      - costNext = 50 USDT (cost of Level 2)
    ///      - 10% of costNext (5 USDT) → piWallet
    ///      - 90% of costNext (45 USDT) → stays in Treasury
    ///      - payout = accumulated - costNext = 162 - 50 = 112 USDT → user
        ...
        returns (address nextBlock)
    {
        ...
        // 1. Split costNext: 10% to PI, 90% stays in Treasury
        uint256 piShare = (costNext * 10) / 100;  // 10% of costNext to PI
        // The remaining 90% (costNext - piShare) stays in Treasury - no transfer needed
        
        // Transfer PI share from Treasury to piWallet
        _safeTransfer(piWallet, piShare);
        
        // 2. Transfer payout (accumulated - costNext) to user
