-- Fix the handle_new_user trigger - remove reference to non-existent is_primary column
...
BEGIN
  ...
  -- Automatically assign admin role to Eduardo and Barrett (identified by email)
  IF LOWER(NEW.email) IN ('eduardorojas@ecrstudio.co', 'barretteduardo@gmail.com') THEN
  ...
  -- If wallet_address exists, create user_wallets entry with referral (WITHOUT is_primary column)
